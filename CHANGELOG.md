...
### Changed
- FORGE surface audit (2026-W16): removed stale WakeService "not yet dispatched" note from schedule docs (runtime ships in #332); added `schedule` row to agent syntax block and components table; added `contradiction` failure type and `downgrade` response to warden docs; fixed "five failure types" → "six" in two places; added 16 missing grammar-level reserved keywords to the keywords reference; added `schedule` block to SKILL.md FORGE preamble; updated stale comment in `examples/basics/schedule.forge`.
