...
### Fixed
- **Wiki Auto Reference & Fact-Check Report UX** — fixed Record serialization leak (`{reference:` / `{report:` prefix) by moving `data.store` into generating stages; improved extraction prompts to produce structured markdown instead of placeholder text; restructured fact-check output with summary table and collapsible per-claim verdicts; added intro context to both pages (#124)
