...
### Added
- **End-to-end wiki acceptance tests** — 37 Rust integration tests covering all 7 wiki subsystems: checker/parse validation, page lifecycle (CRUD + state transitions), HTTP endpoints, search agent, Q&A agent, doc generation flow + fact-check pool, and warden supervision; all tests run with mock provider in under 2 seconds; tests validate all 14 FORGE primitives demonstrated in the wiki (#65)
