...
impl OpenAICompatProvider {
    ...
    fn build_caps(
        ...
        let mut caps = if local {
            ...
            let is_fast = model.contains("8b") || model.contains("7b")
                || model.contains("3b") || model.contains("mini");
            ProviderCapabilities {
                max_context_tokens:        32_768,
                quality_tier:              if is_fast { QualityTier::Fast }
                                           else       { QualityTier::Balanced },
                local:                     false,
        ...
        if let Some(ov) = overrides {
            if let Some(ctx) = ov.max_context_tokens { caps.max_context_tokens = ctx; }
            if let Some(qt)  = &ov.quality_tier      { caps.quality_tier = qt.clone(); }
            if let Some(l)   = ov.local              { caps.local = l; }
            if let Some(c)   = ov.cost_per_1k_input  { caps.cost_per_1k_input_tokens = c; }
            if let Some(c)   = ov.cost_per_1k_output { caps.cost_per_1k_output_tokens = c; }
        }
...
impl LLMProvider for OpenAICompatProvider {
    ...
    async fn complete(
        ...
        messages.push(OAIMessage { role: "user", content: &req.prompt });
        ...
        let http_resp = self.client
            ...
            .map_err(|e| {
                if e.is_timeout() {
                    ProviderError::Timeout { secs: self.timeout.as_secs() }
        ...
        if status == 429 {
            let retry_after = http_resp.headers()
                .get("retry-after")
        ...
        if !status.is_success() {
            let err: OAIError = http_resp.json().await
                .unwrap_or(OAIError {
                    error: OAIErrorBody {
        ...
        let resp: OAIResponse = http_resp.json().await
