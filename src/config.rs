    ...
    fn validate(&self) -> Result<(), ConfigError> {
        if !self.providers.contains_key(&self.llm.default) {
            return Err(ConfigError::UnknownProvider(format!(
                "default provider '{}' not defined in [providers]",
                self.llm.default
            )));
        }
    ...
    fn check_fallback_cycles(&self) -> Result<(), ConfigError> {
        for (name, config) in &self.providers {
            ...
            while let Some(next) = current {
                ...
                visited.push(next);
                current = self.providers.get(next).and_then(|c| c.fallback.as_deref());
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn circular_fallback_detected() {
        ...
        assert!(matches!(
            config.validate(),
            Err(ConfigError::CircularFallback(_))
        ));
    }
