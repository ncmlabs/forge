...
2. For each authoritative area, check whether derived surfaces accurately reflect it:
   ...
   - **Reserved keywords** (two classes, both authoritative):
     - Hard-reserved: the tokens inside the `keyword = @{ ... }` rule. These are never valid as identifiers. `docs/forge-reference.md` lists them as `Reserved keywords: ...`. This list must match the rule exactly — flag any mismatch.
     - Contextually-reserved: the tokens inside the `// ---- Contextual keywords ----` comment block immediately above the `keyword` rule. These are reserved only in specific grammar slots (e.g. `search` in expression position, `can` in contract capability position). `docs/forge-reference.md` lists a *curated* subset as `Additional contextual keywords (reserved in specific contexts): ...`. This list is not exhaustive by design; do NOT auto-add to it. But NEVER remove a token from it unless that token is absent from BOTH the `keyword` rule AND the contextual-keywords annotation block. If a token appears on the hard-reserved doc line but only belongs in the contextual annotation (e.g. `search`), move it rather than delete it.
