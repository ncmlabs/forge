...
fn mock_registry() -> Arc<ProviderRegistry> {
    ...
    let mock = MockProvider::new("mock")
        .with_response(
            "quiz question",
            "What keyword declares a deterministic function in FORGE?",
        )
...
async fn quiz_tutor_full_session() {
    ...
    // Verify memory was initialized
    {
        ...
        assert!(
            matches!(&q_asked.value, Value::Number(n) if *n == 1.0),
            "should have asked 1 question"
        );
        let level = ctx.memory.get("level").unwrap();
        assert!(
            matches!(&level.value, Value::Text(s) if s == "beginner"),
    ...
    // 2. Answer correctly
    println!("\n=== Answering question ===");
    let params: HashMap<String, ConfidentValue> =
        HashMap::from([text_param("student_answer", "pure")]);
    ...
    // Score should have incremented
    {
        ...
        assert!(
            matches!(&score.value, Value::Number(n) if *n >= 1.0),
    ...
    // Questions asked should have gone up
    {
        ...
        let q_asked = ctx.memory.get("questions_asked").unwrap();
        assert!(
            matches!(&q_asked.value, Value::Number(n) if *n >= 3.0),
...
async fn quiz_tutor_requires_guard_rejects_early_answer() {
    ...
    let params: HashMap<String, ConfidentValue> =
        HashMap::from([text_param("student_answer", "tasks")]);
    let result = agent.dispatch("answer", params).await.unwrap();

    // Should get the "No active question" message from the give fail policy
    assert!(
        matches!(result, Some(ref v) if matches!(&v.value, Value::Text(s) if s.contains("No active question"))),
...
async fn quiz_tutor_stuck_detection() {
    ...
    for i in 0..3 {
        ...
        let params: HashMap<String, ConfidentValue> =
            HashMap::from([text_param("student_answer", "I don't know")]);
        let _r = agent.dispatch("answer", params).await.unwrap();
    }
    ...
    println!(
        "Stuck detection test complete — policy would have fired if responses were similar enough"
    );
}
